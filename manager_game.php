    ...
    <?php if ($teams_are_set): ?>
        ...
        <script>
                        ...
                        ${['1PM','2PM','3PM','FOUL','REB','AST','BLK','STL','TO'].map(stat => `
                            <td class="stat-cell" onmouseover="showButtons(this)" onmouseleave="hideButtons(this)">
                                ...
                                <div class="stat-controls">
                                    <button onclick="updateStat('${teamId}', ${idx}, '${stat}', 1)" class="btn-stat-add" ${gameData.gameStatus === 'Final' || (stat === 'FOUL' && isFouledOut) ? 'disabled' : ''}>+</button>
                                </div>
                            </td>`).join('')}
            ...
            function logStatChange(player, teamId, stat) {
                ...
                const logData = { game_id: gameData.gameId, player_id: player.id, team_id: team.id, quarter: currentQuarter, game_clock: gameClockMs, action_type: stat, action_details: details };
                fetch('log_game_action.php', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(logData) }).then(res => res.json()).then(data => { if (data.success && data.log_id) { const newLog = { id: data.log_id, quarter: logData.quarter, game_clock_ms: logData.game_clock, action_details: logData.action_details, is_undone: 0 }; renderLogEntry(newLog); } else { console.error("Failed to log action:", data.error); } }).catch(error => console.error("Error logging action:", error));
            }
            ...
            function logTimeoutAction(teamKey) {
                ...
                fetch('log_game_action.php', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(logData) }).then(res => res.json()).then(data => { if (data.success && data.log_id) { const newLog = { id: data.log_id, quarter: logData.quarter, game_clock_ms: logData.game_clock, action_details: logData.action_details, is_undone: 0 }; renderLogEntry(newLog); } else { console.error("Failed to log timeout:", data.error); } }).catch(error => console.error("Error logging timeout:", error));
            }
            ...
            async function undoAction(logId) {
                ...
                if (result.success) { alert("Action undone. Page will now refresh."); location.reload(); } else { alert("Failed to undo action: " + result.error); }
            }
            ...
            async function redoAction(logId) {
                ...
                const result = await response.json();
                if (result.success) { alert("Action redone. Page will now refresh."); location.reload(); } else { alert("Failed to redo action: " + result.error); }
            }
            ...
            window.addEventListener("DOMContentLoaded", () => {
                ...
                document.querySelectorAll(".timeout-click").forEach(el => {
                    el.addEventListener("click", async () => {
                        ...
                        else timeoutPeriod = currentQuarter;
